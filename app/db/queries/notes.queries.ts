...
export const insertNote = async (db: SQLiteDatabase, note: CUNoteFormData): Promise<CUNoteFormData> => {
    note.id = uuid.v4();
    const query = "INSERT INTO notes (id, title, content, favorite, color,pinned) VALUES (?, ?, ?, ?, ?, ?)";
    const params = [note.id, note.title, note.content, note.favorite, note.color,note.pinned];
    await db.executeSql(query, params);
    return note;
}

export const getNotes = async (db: SQLiteDatabase) => {
    const query = "SELECT * FROM notes ORDER BY pinned DESC ,updated_at DESC";
